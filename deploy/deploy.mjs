...
async function main() {
  ...
  await run(
    `ssh ${SSH_OPTS} ${USER}@${SERVER} "cd ${REMOTE} && export NODE_ENV=production && export DATABASE_URL='${DATABASE_URL}' && export NEXT_SERVER_ACTIONS_ENCRYPTION_KEY='${SERVER_ACTIONS_KEY}' && mkdir -p prisma && rm -rf node_modules && npm ci --no-audit --no-fund && ./node_modules/.bin/prisma migrate deploy && ./node_modules/.bin/prisma generate && npm run db:seed && npm run build"`
  );
