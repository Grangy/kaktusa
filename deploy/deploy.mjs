...
async function main() {
  ...
  await run(
    `ssh ${SSH_OPTS} ${USER}@${SERVER} "cd ${REMOTE} && export NODE_ENV=production && export DATABASE_URL='${DATABASE_URL}' && export NEXT_SERVER_ACTIONS_ENCRYPTION_KEY='${SERVER_ACTIONS_KEY}' && export NEXT_DISABLE_TURBOPACK=1 && mkdir -p prisma && rm -rf node_modules && npm ci --no-audit --no-fund --include=dev --legacy-peer-deps && node node_modules/prisma/build/index.js db push && node node_modules/prisma/build/index.js generate && npm run db:seed && npm run build"`
  );
