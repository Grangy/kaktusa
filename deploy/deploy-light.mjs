...
async function main() {
  ...
  await run(
    `ssh ${SSH_OPTS} ${USER}@${SERVER} "cd ${REMOTE} && export NODE_ENV=production && export DATABASE_URL='${DATABASE_URL}' && export NEXT_SERVER_ACTIONS_ENCRYPTION_KEY='${SERVER_ACTIONS_KEY}' && export NEXT_DISABLE_TURBOPACK=1 && (PREV=\\\$(cat /tmp/kaktusa_prev_ref 2>/dev/null); DEPS=\\\$(git diff --name-only \\\"\\$PREV\\\" HEAD 2>/dev/null | grep -E 'package(-lock)?\\\\.json' || true); NEED=0; node -e \\\"require.resolve('nodemailer');require.resolve('qrcode')\\\" >/dev/null 2>&1 || NEED=1; if [ -n \\\"\\$DEPS\\\" ] || [ ! -d node_modules ] || [ \\\"\\$NEED\\\" = 1 ]; then npm ci --no-audit --no-fund --include=dev --legacy-peer-deps; fi) && node node_modules/prisma/build/index.js db push && node node_modules/prisma/build/index.js generate && npm run db:seed && npm run build && mkdir -p public/photos && cp -r .next/static .next/standalone/.next/ && cp -r public .next/standalone/ && rm -rf .next/standalone/public/photos && ln -sfn ${REMOTE}/public/photos .next/standalone/public/photos"`
  );
